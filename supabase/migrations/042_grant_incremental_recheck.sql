-- PR8: expose immutable diagnostic runs to the diagnostic authority so it can
-- derive current results, incremental scope and convergence without mutating
-- historical Findings.
...
COMMENT ON FUNCTION public.list_grant_diagnostic_runs(UUID, UUID) IS
  'Owner-scoped immutable run history used to derive incremental recheck scope and convergence.';

-- Add stage-aware, multi-request Grant Assistant execution telemetry.
-- Stores identifiers, booleans, hashes and aggregate usage only; never
-- sensitive request or generated content.

ALTER TABLE public.grant_model_calls
  ADD COLUMN IF NOT EXISTS provider_request_ids TEXT[],
  ADD COLUMN IF NOT EXISTS failure_stage TEXT,
  ADD COLUMN IF NOT EXISTS request_dispatched BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS usage_known BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS context_manifest_hash TEXT;

UPDATE public.grant_model_calls
SET
  provider_request_ids = CASE
    WHEN provider_request_id IS NULL THEN NULL
    ELSE ARRAY[provider_request_id]
  END,
  request_dispatched = provider_request_id IS NOT NULL,
  usage_known = (input_tokens + output_tokens + reasoning_tokens) > 0
WHERE provider_request_ids IS NULL;

ALTER TABLE public.grant_model_calls
  DROP CONSTRAINT IF EXISTS grant_model_calls_failure_stage_check,
  DROP CONSTRAINT IF EXISTS grant_model_calls_context_manifest_hash_check;

ALTER TABLE public.grant_model_calls
  ADD CONSTRAINT grant_model_calls_failure_stage_check CHECK (
    failure_stage IS NULL OR failure_stage IN (
      'memory_build', 'semantic_planning', 'context_admission',
      'original_retrieval', 'answer_generation', 'persistence'
    )
  ),
  ADD CONSTRAINT grant_model_calls_context_manifest_hash_check CHECK (
    context_manifest_hash IS NULL OR context_manifest_hash ~ '^[a-f0-9]{64}$'
  );
...
CREATE OR REPLACE FUNCTION public.finish_grant_model_call(
  ...
  p_failure_category TEXT, p_failure_stage TEXT, p_request_dispatched BOOLEAN,
  p_usage_known BOOLEAN, p_context_manifest_hash TEXT,

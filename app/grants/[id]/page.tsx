...
import { isGrantAiPatchEnabled, isGrantWorkspaceEnabled } from "@/lib/grants/server/config";

export default async function GrantEditorPage({ params }: { params: Promise<{ id: string }> }) {
  if (!isGrantWorkspaceEnabled()) notFound();
  const { id } = await params;
  return <GrantStructuredEditor documentId={id} aiPatchEnabled={isGrantAiPatchEnabled()} />;
}

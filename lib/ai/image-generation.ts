...
export function isGptImageRequest(query: string): boolean {
  ...
  const wantsEditableStructure =
    /(流程图|鱼骨图|时间轴|技术路线图|框架图|结构图|思维导图|柱状图|折线图|饼图|散点图|热图|SVG|可编辑|flowchart|fishbone|ishikawa|timeline|framework|mind ?map|bar chart|line chart|pie chart|scatter|heatmap|svg)/i.test(
      text,
    );

  return wantsRasterImage && !wantsEditableStructure;
}
...
export function buildImagePrompt(messages: ChatMessage[]): string {
  ...
  return [
    ...
    "- Clean scientific SaaS style, refined layout, strong hierarchy, restrained colors, readable text.",

...
export const GrantAssistantMessageSchema = z.object({
  ...
  citations: z.array(z.object({ citationId: z.string(), sourceAlias: z.string().optional(), sourceType: z.enum(["document_selection", "edit_candidate", "evidence", "academic_source", "web_source"]), label: z.string(), url: z.string().url().optional() }).strict()).max(24).default([]),

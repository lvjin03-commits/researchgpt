...
assert.match(panelSource, /setMessages\(\(items\) => \[\.\.\.items, \{ messageId: `\$\{turnId\}:user`/);
assert.match(panelSource, /setInput\(""\);\s*setPendingAssistantTurnId\(turnId\);/);
assert.match(panelSource, /aria-label="Grant AI 正在回复"/);
assert.match(panelSource, />发送<\/button>/);
assert.doesNotMatch(panelSource, /\{busy \? "处理中…" : "发送"\}/);

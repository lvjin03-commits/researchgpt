...
### Research-task source provenance target

ADR 0055 distinguishes academic evidence from general-web discovery before
multi-query research behavior is enabled. A structured academic Provider is the
only owner allowed to populate paper abstract and bibliographic fields. OpenAI
native web search may populate bounded `citation_context`, but that text must
never be labeled or presented as an abstract. Missing abstract, DOI, venue and
publication time values remain missing rather than being model-completed.

Each new source record declares an evidence-text provenance kind: `abstract`,
`citation_context`, `official_excerpt` or `metadata_only`. Provider-owned
identity and the deterministic trust-registry classification remain immutable.
One Grant Web Source Acquisition Service normalizes and deduplicates results by
DOI, canonical URL and normalized title while preserving every provenance
record. Models receive only the selected normalized projection and cannot
assign source identity, quality tier or provenance.

This target does not authorize paper full-text acquisition. Structured
abstracts, bibliographic metadata, publisher-displayed abstract fields and
bounded official excerpts are the maximum content boundary until a separate
impact analysis and ADR approve an exception.

The V2 research-source assessment proposal contains semantic fields only:
`mechanismSummary`, `quantitativeFindings`, `applicationRelation`,
`evidenceLimitations`, `disposition` and `reason`. Publication year, DOI,
Provider identity, primary source and trust tier are joined by the program from
the current source group and are never model-authored. Every quantitative
finding names one or more member source IDs, and every numeric token must occur
in the admitted evidence text for those members. Missing quantitative evidence
cannot be repaired by inference.

Research-gap comparison reuses the scientific diagnostic coverage disposition:
`residual_gap_found`, `verified_no_residual_gap` or `unable_to_verify`. The model
may reference only execution-local application locations; the program resolves
them against the frozen Revision context. `latestDevelopment` is copied from the
already validated source assessment rather than regenerated by the comparison
model. `verified_no_residual_gap` publishes no gap, insufficiency reason or
recommendation. `unable_to_verify` publishes only its bounded reason. Only
`residual_gap_found` may publish a modification recommendation, and it must also
declare whether existing design was found. `found` requires resolved application
...
application-specific and at least one mechanism, dynamic-interface or
quantitative-performance dimension. Every candidate still passes the existing
egress policy independently before external dispatch.

The model does not decide whether research is complete. A deterministic
next-step policy stops early only after at least three queries and sufficient
validated coverage: both required dimensions, one scientific-depth dimension,
five relevant sources, three structured abstracts, two recent sources, one
source-supported quantitative finding and completed application comparisons.
If the plan is exhausted first, final delivery carries explicit program-derived
limitations rather than inventing missing evidence.

Paid research phases enter one budget adapter before model or hosted-search
dispatch. The adapter resolves the registered Operation Policy, validates the
planned token/tool/attempt/time limits, obtains a maximum-charge quote from the
site-wide Point Billing Service and only then requests atomic phase admission.
OpenAlex requests with no billable provider/model usage remain outside point
reservation but still require the existing egress audit.

Research-gap comparison is registered as `grant.web_gap.compare` with policy
`grant-web-gap-compare-v1`; it is distinct from source assessment. Declining an
increase first persists the decision-envelope waiver, then executes only the
protected, zero-tool, single-attempt delivery Operation. That Operation always
settles as partial delivery and may use only persisted current artifacts.
...
## Web-grounded answer budget target

ADR 0054 replaces the fixed 50-point, platform-absorbed-overage policy only
after its additive migration and rollout gates are complete. The target uses
one persisted, resumable budget aggregate within the existing Grant Assistant
and site-wide Point Billing authorities.

Each initial budget and increase is an explicit, idempotent user authorization.
Decision, execution and final-delivery envelopes have separate hard limits.
Before every provider or paid tool call, the coordinator must prove that
settled cost, the active call maximum, remaining admitted decision cost and the
reserved final-delivery maximum fit within total authorized points. Planning is
not free: a model-based next-step decision is itself a registered, pre-admitted
Operation.

The aggregate may pause as `awaiting_budget` with zero further provider calls.
The user may explicitly increase the budget or request bounded delivery from
existing results. Final delivery is one no-tool, one-attempt Operation with hard
input/output limits; its reserve cannot be consumed by earlier work. A failed
model delivery falls back only to a deterministic artifact manifest and cannot
invent a scientific synthesis.

Paused tasks retain the delivery reserve for 48 hours. Expiry and release are
owned by the ledger/reconciliation service, and account UI only projects their
state. Resume revalidates current Revision, affected node hashes, source
fingerprints and Evidence authorization. Independent public-source artifacts
may remain reusable; stale document-relative analysis may not. Any recompute is
subject to a new budget check.

The deployed fixed-cap path remains authoritative until this target is fully
implemented, migrated and explicitly activated. Both budget mechanisms must not
remain selectable after migration.

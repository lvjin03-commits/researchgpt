...
| Grant Assistant execution mode and memory-planned execution | Grant Assistant Memory Pipeline | Maps validated semantic access to memory discussion, targeted original verification or hierarchical full review; runs the selected mode as one candidate turn and reports aggregate provider usage; no parallel full-document gateway or billing authority |
| Grant model failure facts and trace | Grant Model Executor | Persists category, stage, dispatch state, usage-known state, provider request IDs and usage; presentation cannot reclassify these facts |
| Grant model failure presentation | Grant Model Failure Presenter | Maps recorded category and stage to HTTP status, retryability and safe user text; never decides retries, billing or provider facts |

...
export function GrantDocumentCanvas(props: Props) {
  const section = props.snapshot.sections.find((candidate) => candidate.sectionId === props.selectedSectionId);
  const nodes = section
    ? section.nodeIds.map((nodeId) => props.snapshot.nodes.find((node) => node.nodeId === nodeId)).filter(Boolean) as CanonicalGrantSnapshot["nodes"]
    : [];
      ...
      <div className="mx-auto min-h-[900px] max-w-3xl rounded-sm border border-slate-200 bg-white px-6 py-10 shadow-sm sm:px-14 sm:py-16">
        {section && (
          <>
            <input
              aria-label="章节标题"
              className="research-focus mb-8 w-full rounded-lg border border-transparent px-2 py-2 text-2xl font-semibold text-slate-900 hover:border-slate-200"
              value={section.title}
              onChange={(event) => props.onSectionTitleChange(event.target.value)}
            />

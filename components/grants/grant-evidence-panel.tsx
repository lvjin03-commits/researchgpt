    ...
    <article className="rounded-xl border border-slate-200 bg-white p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-xs font-semibold text-slate-800">{props.resource.source.title}</p>
          <p className="mt-1 text-[11px] text-slate-500">
            {provenanceLabels[props.resource.source.provenanceType]} · {props.resource.cards.length} 张证据卡
          </p>
        </div>
        <span className={`rounded-full px-2 py-0.5 text-[10px] ${active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
          {active ? "可用" : props.resource.source.status === "revoked" ? "已撤权" : "删除中"}
        </span>
      </div>

      <details className="mt-3">
        <summary className="cursor-pointer text-[11px] font-medium text-[#245d82]">查看证据卡与授权</summary>
        <div className="mt-2 space-y-2">
          {props.resource.cards.slice(0, 3).map((card) => (
            <blockquote key={card.cardId} className="max-h-28 overflow-y-auto rounded-lg bg-slate-50 p-2 text-[11px] leading-5 text-slate-600">
              {card.excerpt}
            </blockquote>
          ))}
          {props.resource.cards.length > 3 && <p className="text-[10px] text-slate-400">另有 {props.resource.cards.length - 3} 张证据卡</p>}
          <div className="space-y-1.5 border-t border-slate-100 pt-2">
            {permissionLabels.map(([key, label]) => (
              <label key={key} className="flex items-center gap-2 text-[11px] text-slate-700">
          ...
          {active && (
            <button
              ...
              className="w-full rounded-lg bg-[#245d82] px-2 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50"
            >保存授权</button>
          )}
          <div className="flex gap-2">
            {active && (
              <button
                ...
                className="flex-1 rounded-lg border border-amber-300 px-2 py-1.5 text-[11px] font-semibold text-amber-800 disabled:opacity-50"
              >撤销全部授权</button>
            )}
            <button
              type="button"
              ...
              className="rounded-lg border border-red-200 px-2 py-1.5 text-[11px] font-semibold text-red-700 disabled:opacity-50"
            >删除</button>
          </div>
...
export function GrantEvidencePanel({ documentId, enabled }: { documentId: string; enabled: boolean }) {
  ...
  return (
    <section className="mt-6 border-t border-slate-200 pt-4" aria-label="项目资料">
      ...
      <p className="mt-2 text-[11px] leading-5 text-slate-500">资料不会自动进入 AI。读取、推理与引用权限相互独立。</p>
      <div className="mt-3 grid gap-2">
        <select value={provenanceType} onChange={(event) => setProvenanceType(event.target.value as keyof typeof provenanceLabels)} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px]">
          {Object.entries(provenanceLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
        </select>
        <select value={sensitivity} onChange={(event) => setSensitivity(event.target.value)} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px]">
        ...
        <button type="button" disabled={busy} onClick={() => fileInput.current?.click()} className="rounded-lg border border-[#245d82] px-3 py-2 text-xs font-semibold text-[#245d82] disabled:opacity-50">
          {busy ? "解析并保存中…" : "+ 上传本地资料"}
        </button>
      </div>

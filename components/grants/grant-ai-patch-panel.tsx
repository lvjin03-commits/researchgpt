    ...
    <section className="mt-4 rounded-xl border border-blue-100 bg-blue-50/50 p-3">
      ...
      {props.evidencePatchEnabled && (
        <fieldset className="mt-3 rounded-lg border border-slate-200 bg-white p-3">
          ...
          <p className="mb-2 text-xs leading-5 text-slate-500">只显示已允许发送摘录并用于推理的资料；不会自动生成正式引用编号。</p>
          <div className="space-y-1.5">
            {availableEvidence.map((resource) => (
              <label key={resource.source.sourceId} className="flex items-start gap-2 text-xs leading-5 text-slate-700">
                <input
                  type="checkbox"
            ...
            {availableEvidence.length === 0 && <p className="text-xs text-slate-400">暂无可选资料。请先在左侧上传并授权。</p>}
            {selectedSourceIds.length >= 8 && <p className="text-xs text-amber-700">一次修改最多使用 8 项资料。</p>}
          </div>
        </fieldset>
      )}
      <button
        ...
        className="mt-2 w-full rounded-lg bg-[#155eef] px-3 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
      >
        ...
        <div className="mt-3 space-y-2" aria-label="AI 修改差异预览">
          ...
          {proposal && proposal.evidenceBindings.length > 0 && (
            <div className="rounded-lg border border-blue-100 bg-white p-3">
              <p className="text-xs font-semibold text-[#245d82]">本次修改依据</p>
              <ul className="mt-1 list-inside list-disc text-xs leading-5 text-slate-600">
          ...
          <div className="flex gap-2">
            <button type="button" disabled={busy} onClick={() => void decide("accept")} className="flex-1 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">接受并写入</button>
            <button type="button" disabled={busy} onClick={() => void decide("reject")} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">拒绝</button>
          </div>
          <p className="text-xs leading-5 text-amber-700">AI 仅生成预览；确认后才创建新版本。</p>
        </div>
      )}
    </section>
  );
